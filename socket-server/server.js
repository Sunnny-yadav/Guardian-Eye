...
io.on("connection", (socket) => {
  ...
  socket.on("prediction-complete", (data) => {
    if (!data?.disaster.region) {
      console.log("⚠️ No region provided for prediction broadcast");
      return;
    }

    io.to(data?.disaster.region).emit("prediction-results", data);
    console.log(`📊 Prediction results broadcasted to region: ${data?.disaster.region}`);
  });
...
app.post("/api/emit-disaster-alert", (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to emit alert", success: false });
  }
});
